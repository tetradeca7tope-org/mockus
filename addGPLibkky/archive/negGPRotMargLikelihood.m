function [negNlml, nAG] = negGPRotMargLikelihood(sigmaSms, sigmaPrs, ...
  decomposition, A, X, y, meanFuncs, commonMeanFunc, noises, commonNoise) 

  if nargout == 2,
    [nlml, AG] = GPRotMargLikelihood(sigmaSms, sigmaPrs, decomposition, A, ...
      X, y, meanFuncs, commonMeanFunc, noises, commonNoise);
  else
    nlml = GPRotMargLikelihood(sigmaSms, sigmaPrs, decomposition, A, ...
      X, y, meanFuncs, commonMeanFunc, noises, commonNoise);
    AG = [];
  end
  negNlml = -nlml;
...
function [nlml, AG] = GPRotMargLikelihood(sigmaSms, sigmaPrs, decomposition, ...
  A, X, y, meanFuncs, commonMeanFunc, noises, commonNoise)
