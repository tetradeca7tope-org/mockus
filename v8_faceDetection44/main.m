...
function [] = main(numExperiments,...
numIters,numXTrain,guessRange,numDiRect)
...
numXTest = 1;
numDimsPerGroupCands = [44 1 2 4 8 11 15 22]';
doubleParams = true;
...
numDiRectEvals = numDiRect; %min(20000, max(100*numDims, 500));
numInitPts = 10;
...
% Introduce upper bound on each stage
load('upperBounds');
guess = maxMaxStages';
vjUpperBounds = [guess-guessRange guess+guessRange];;
vjBounds = [vjBounds; vjUpperBounds];
...
% Introduce nominal upper bound on each stage
nominalUpperParams = maxMaxStages;
nominalParams = [nominalParams  nominalUpperParams];

normalizedNominalParams = getNormParams(nominalParams, vjBounds);
func = @(t) vjWrap(XTrain, YTrain, HaarCascade, t, vjBounds,vjOptions,0);
...
boParams.sigmaPrRange = [5 100]; %[5 25];
...
%[s,a] = func(normalizedNominalParams);
...
for expIter = 1:numExperiments
  ...
  save(saveFileName, 'numDims', 'numDimsPerGroupCands', ...
    'numIters', 'totalNumQueries', 'numdCands', ...
    'boAddHistories', 'randHistories',  ...
    'boAddMaxVals', 'randMaxVals',  ...
    'boAddCumRewards', 'randCumRewards',  ...
    'boAddQueryPts', 'randQueryPts',  ...
    'boAddTimes', 'randTimes', ...
    'guessRange', 'numDiRect');
